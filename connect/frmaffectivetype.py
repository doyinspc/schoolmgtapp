# -*- coding: utf-8 -*-
"""
Created on Thu Aug  9 12:14:57 2018
    ...
    def button_click(self, a, b):
        ...
        try:
            if(len(s1) > 0 and len(s2) > 0 and len(s3) > 0 and len(s4) > 0):
                y = { 'name':s1, 'abbrv':s2, 'pubID':12, 'subID':self.a,  'description':fil ,'active':0}
                w = g.insert('datas', y) 
                print(w)
            else:
                pass
...
class EditForm(QDialog): 
    ...
    def __init__(self, sid, parent=None):
        ...
        try: 
            ...
            pagetitle = str(details[2]) +' Attitude/Behavior' 
        except:
            details = []
            pagetitle = 'None'
        ...
        groupBox = QGroupBox('Edit Attitude/Behavior')
        groupBox.setLayout(layout)
    ...
    def button_click(self, a, c, b):
        ...
        self.form = AffectiveTypeForm(self.c)
        self.form.show()
        self.close()
        #self.lunchForm()
