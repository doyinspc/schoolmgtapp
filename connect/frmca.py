...
class CaForm(QDialog):
    pagetitle = 'Assessment Manager'
    def __init__(self, parent=None):
        super(CaForm, self).__init__(parent)
...
class AddForm(QDialog):    
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Assessments Manager")
    ...
    def button_click(self, a):
        ...
        try:
            if(len(s1) > 0) and (len(s2) > 0):
                y = { 'name':s1.lower(), 'abbrv':s2.lower(), 'pubID':7, 'active':0}
                g.insert('datas', y)
            else:
                pass
        except:
...
class EditForm(QDialog): 
    ...
    def __init__(self, sid, parent=None):
        ...
        self.connect(self.pb1, SIGNAL("clicked()"),lambda: self.button_close(self))
        self.setWindowTitle("Assessment Manager")
