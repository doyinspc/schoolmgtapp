...
from frmexpensetype import ExpenseTypeForm

class ExpenseForm(QDialog):
    pagetitle = 'Expenses Manager'
    def __init__(self, parent=None):
        super(ExpenseForm, self).__init__(parent)
    ...
    def lunchUnitForm(self, a, b):
        b.close()
        self.a = a
        self.form = ExpenseTypeForm(self.a)
        self.form.show()
    ...
    def button_click(self, a):
        ...
        try:
            if(len(s1) > 0) and (len(s2) > 0):
                y = { 'name':s1.lower(), 'abbrv':s2.lower(), 'pubID':15, 'active':0}
                g.insert('datas', y)
