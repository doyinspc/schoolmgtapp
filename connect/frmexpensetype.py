...
class ExpenseTypeForm(QDialog):
    
    def __init__(self, sid, parent=None):
        super(ExpenseTypeForm, self).__init__(parent)
        self.sid = sid
        ko = 0
        layout1 = QGridLayout()
        try: 
...
class AddForm(QDialog):    
    def __init__(self, sid,  parent=None):
        ...
        try: 
            details = self.pullDetails(self.sid)
            pagetitle = str(details[1]) +' Expences' 
        except:
            details = []
            pagetitle = 'None'
    ...
    def button_click(self, a, b):
        ...
        try:
            if(len(s1) > 0 and len(s2) > 0 ):
                y = { 'name':s1, 'abbrv':s2, 'pubID':16, 'subID':self.a,  'description':0l ,'active':0}
                g.insert('datas', y) 
            else:
                pass
        except:
...
class EditForm(QDialog): 
    ...
    def __init__(self, sid, parent=None):
        ...
        try: 
            ...
            pagetitle = str(details[2]) +' Expences' 
        except:
            details = []
            pagetitle = 'None'
        ...
        groupBox = QGroupBox('Edit Expenses Type')
        groupBox.setLayout(layout)
