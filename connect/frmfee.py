# -*- coding: utf-8 -*-
"""
Created on Sat Oct 27 12:22:54 2018
...
class FeeForm(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        groupBox1 = QGroupBox('All Fee')
        groupBox1.setLayout(layout1)
        
        self.pb = QPushButton()
        self.pb.setObjectName("Add")
        self.pb.setText("Add Fee Name")
...
class AddForm(QDialog):    
    def __init__(self, parent=None):
        ...
        groupBox = QGroupBox('Add Fee Name')
        ...
        self.setWindowTitle("Fee Names Manager")
    ...
    def button_click(self, a):
        ...
        try:
            if(len(s1) > 0) and (len(s2) > 0):
                y = { 'name':s1.lower(), 'abbrv':s2.lower(), 'pubID':17, 'active':0}
                g.insert('datas', y)
            else:
                pass
        except:
...
class EditForm(QDialog): 
    ...
    def __init__(self, sid, parent=None):
        ...
        groupBox = QGroupBox('Edit Assessment')
        groupBox.setLayout(layout)
        
        grid = QGridLayout()
        grid.addWidget(groupBox, 0, 0)
        self.setLayout(grid)
        self.connect(self.pb, SIGNAL("clicked()"), lambda: self.button_click(data['id'], self))
        self.connect(self.pb1, SIGNAL("clicked()"),lambda: self.button_close(self))
        self.setWindowTitle("Fee Names Manager")
