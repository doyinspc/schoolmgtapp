...
import json
class PsycoTypeForm(QDialog):
    
    def __init__(self, sid, parent=None):
        super(PsycoTypeForm, self).__init__(parent)
        self.sid = sid
        ko = 0
        layout1 = QGridLayout()
        try: 
    ...
    def button_click(self, a, b):
        ...
        try:
            if(len(s1) > 0 and len(s2) > 0 and len(s3) > 0 and len(s4) > 0):
                y = { 'name':s1, 'abbrv':s2, 'pubID':10, 'subID':self.a,  'description':fil ,'active':0}
                w = g.insert('datas', y) 
                print(w)
            else:
                pass
...
class EditForm(QDialog): 
    ...
    def button_click(self, a, c, b):
        ...
        self.form = PsycoTypeForm(self.c)
        self.form.show()
        self.close()
        #self.lunchForm()
