...
from frmsubjectcomp import CompForm
...
class AddForm(QDialog):    
    def __init__(self, parent=None):
        ...
        groupBox = QGroupBox('Add Subject')
        ...
        self.setWindowTitle("Subjects Manager")
    ...
    def button_click(self, a):
        ...
        try:
            if(len(s1) > 0) and (len(s2) > 0):
                y = { 'name':s1.lower(), 'abbrv':s2.lower(), 'pubID':3, 'active':0}
                g.insert('datas', y)
            else:
                pass
        except:
...
class EditForm(QDialog): 
    ...
    def __init__(self, sid, parent=None):
        ...
        groupBox = QGroupBox('Edit Subject')
        groupBox.setLayout(layout)
